...
class Analysis1(spark: SparkSession, hiveStatement: Statement, dataFrame: DataFrame) { 
    ...
    def mostPopularProductByCategory() {        // displays the most popoular product name and which category it belongs to with the quantity that was sold.
        ...
        val df2 = spark.sql(s"SELECT product_name, product_category, SUM(qty) as quantitySold FROM popularProduct GROUP BY product_category, product_name ORDER BY quantitySold DESC LIMIT 1")
    ...
    def mostPopularProductByCategoryHive() {    // Using Hive: displays the most popular product and which category it belongs to with the quantity that was sold.
        val query = s"""
        SELECT product_name, product_category, SUM(qty) as quantitySold
        FROM $tableName
        ...
        if (result.next()) {
            System.out.println(f"Product Name\tProduct Category\tQuantity Sold\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n${result.getString(1)}\t\t${result.getString(2)}\t\t${result.getString(3)}");
        }
        println("\n############################################################")
    }
    ...
    def highestRevenueProduct(){      // displays the product and its category that has the highest revenue and that revenue.
        ...
        val df2 = spark.sql(s"SELECT product_name, product_category, SUM(qty * price) as revenue FROM revenueProduct GROUP BY product_category, product_name ORDER BY revenue DESC LIMIT 1")
    ...
    def highestRevenueProductHive() { // Using Hive: displays the product and its category that has the highest revenue and that revenue.
        val query = s"SELECT product_name, product_category, SUM(qty * price) as revenue FROM $tableName GROUP BY product_category, product_name ORDER BY revenue DESC LIMIT 1"
        println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\nFinding the Product with the Highest Revenue...\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        val result = hiveStatement.executeQuery(query)
        
        if (result.next()) {
            System.out.println(f"Product Name\tProduct Category\tProduct Revenue\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n${result.getString(1)}\t\t${result.getString(2)}\t\t$$${result.getString(3).toFloat}%.2f");
        }
        println("\n############################################################")
    }
}