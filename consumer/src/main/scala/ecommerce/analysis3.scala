...
class Analysis3(spark: SparkSession, hiveStatement: Statement, dataFrame: DataFrame) { 
    ...
    def transactionMinAmountHive(): Unit = {
        ...
        println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Finding the minimum transaction amount~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        if (result.next()) {
    ...
    def transactionMaxAmountHive(): Unit = {
        val query = s"""SELECT customer_id, customer_name, COUNT(order_id) as `total_orders`, SUM(price * qty) as `total_price`
                    ...
                    """
        val result = hiveStatement.executeQuery(query)
        println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Finding the maximum transaction amount~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        if (result.next()) {
            System.out.println(f"${result.getString(1)}\t${result.getString(2)}\t${result.getString(3)}\t${"$" + result.getString(4).toString}")
        }
    }
    ...
    def transactionAvgAmount(): Unit =  {
        ...
        val df4 = spark.sql(s"SELECT COUNT(order_id) as `total_orders`, ROUND(AVG(totalPrice)) as `avg_txn_amt` FROM (SELECT order_id, COUNT(order_id) as totalOrders, customer_id, customer_name,  SUM(price * qty) as totalPrice FROM ecommerce GROUP BY customer_id ORDER BY totalPrice DESC)")
        df4.show()
    }

    def transactionAvgAmountHive(): Unit = {
        val query = s"""SELECT COUNT(order_id) as `total_orders`, ROUND(AVG(totalPrice)) as avg_txn_amt`
                        FROM 
                        (
                            SELECT order_id, COUNT(order_id) as totalOrders, customer_id, customer_name,  SUM(price * qty) as totalPrice
                            FROM ecommerce
                            ...
                            """
            val result = hiveStatement.executeQuery(query)
            println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Finding the average transaction amount~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            if (result.next()) {
